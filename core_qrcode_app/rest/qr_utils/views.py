...
class QRCodeDecode(APIView):
    ...
    def get(self, request):
        ...
        try:
            # Generate a QR Code.
            return HttpResponse(open("{0}/static/core_qrcode_app/user/html/qrcode.html".format(Path(Path(os.path.dirname(os.path.realpath(__file__))).parent).parent), "r").read(), content_type="text/html")
        except Exception as api_exception:
            content = {'message': str(api_exception)}
            return Response(content, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
