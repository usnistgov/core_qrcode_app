...
class QRCodeDecode(APIView):
    ...
    def get(self, request):
        ...
        try:
            # Generate a QR Code.
            return Response(open("{0}/templates/core_qrcode_app/capture.html".format(os.path.dirname(os.path.realpath(__file__))), "r").read(), content_type="text/html", status=status.HTTP_200_OK)
        except Exception as api_exception:
            content = {'message': str(api_exception)}
            return Response(content, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
